...
interface DoctorServiceListProps {
  doctorId: number;
}

function DoctorServiceList({ doctorId }: DoctorServiceListProps) {
  ...
  const {
    ...
    queryKey: ['doctorServicesList', doctorId],
            ...
            {isLoading ? (
              ...
              services.map((service) => (
                <TableRow key={service.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  ...
                  <TableCell>{service.durationMinutes} {i18n.language.startsWith('hu') ? 'perc' : 'min'}</TableCell>
